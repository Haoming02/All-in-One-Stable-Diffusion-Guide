...
2. **Update the Project:** In the future, there may be newer versions of the webui available. I highly recommend you to back up the old version *(**eg.** Rename the project folder)* and clone the whole project again to avoid complications.
3. **COMMANDLINE ARGS:** Go to the project folder, right click the file `webui-user.bat` and press `Edit`. Then:
    ...
    - Add `--no-half-vae` after `COMMANDLINE_ARGS=` to prevent NaN images from generating
...
4. **(Optional)(Recommended)** Download a Checkpoint model and put it into `~\stable-diffusion-webui\models\Stable-diffusion` first.
...
- You can add `--ckpt <path to model>` and `--vae-path <path to vae>` after `COMMANDLINE_ARGS=` to specify which models to start with. This way, you can for example: duplicate and rename the `.bat` to `webui-user-realistic.bat` and `webui-user-anime.bat` then edit the args, so that they will start with their respective models.
- You can add `--api` after `COMMANDLINE_ARGS=` to allow other programs to talk to it. See the official [Documents](https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/API) for more info.
